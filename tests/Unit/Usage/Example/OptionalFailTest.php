...
it('constructs OptionalFail from scalars and exposes typed values', function (): void {
    $vo = OptionalFail::fromScalars(id: 1, firstName: 'Foobar', height: 170.0);
...
it('checks string cast', function (): void {
    $voInt = OptionalFail::fromScalars(id: 1, firstName: 'Test', height: 99.0);
...
it('fails early when height is negative', function (): void {
    expect(fn() => OptionalFail::fromScalars(id: 1, firstName: 'Foobar', height: -10.0)->getHeight()->value())
...
it('accepts int/float/numeric-string heights and preserves string formatting via fromString casting', function (): void {
    $asInt = OptionalFail::fromScalars(id: 1, firstName: 'Foobar', height: 170.0);
...
it('null firstName produces Undefined via tryFromMixed while height succeeds', function (): void {
    $obj = OptionalFail::fromScalars(id: 1, firstName: null, height: 180.0);
...
it('invalid id throws IntegerTypeException with exact message', function (): void {
    expect(fn() => OptionalFail::fromScalars(id: 0, firstName: 'Name', height: 100.0))
...
it('non-numeric height string throws FloatTypeException from assertFloatString', function (): void {
    expect(fn() => OptionalFail::fromScalars(id: 1, firstName: 'Name', height: 'abc')->getHeight()->value())
        ->toThrow(UndefinedTypeException::class);
});

it('jsonSerialize returns associative array of strings when all values are present', function (): void {
    $vo = OptionalFail::fromScalars(id: 1, firstName: 'Foo', height: 170.0);
