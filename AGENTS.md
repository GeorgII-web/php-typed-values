...
- NEVER normalize or format the input data in type-classes, use the given value as it is (no trim(), strtolower(),
  etc.).
...
- Create a new exception class in `src/Exception/{TypeName}/` or deeper directory, name has reverted order of words, for
  `IntegerPositive` it's `PositiveIntegerTypeException`.
- Create a new test class in `tests/Unit/{TypeName}/{TypeName}Test.php` file. Make 100% code/type/mutation coverage,
  copy the nearest tests file cases, use "Fix test" rules.
...
- Run
  `docker-compose exec -ti php ./vendor/bin/pest tests/Unit/"newTypeClassName" --mutate --covered-only --parallel --min=100`
