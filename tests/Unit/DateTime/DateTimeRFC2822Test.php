...
use PhpTypedValues\DateTime\DateTimeRFC2822;
use PhpTypedValues\Exception\DateTime\RFC2822DateTimeTypeException;
use PhpTypedValues\Undefined\Alias\Undefined;

covers(DateTimeRFC2822::class);

describe('DateTimeRFC2822', function () {
    describe('Creation', function () {
        describe('fromDateTime', function () {
            it('returns same instant and toString is RFC2822', function () {
        ...
        describe('fromString', function () {
            it('parses valid RFC2822', function () {
                $vo = DateTimeRFC2822::fromString('Tue, 31 Dec 2030 23:59:59 +0000');
                expect($vo->toString())->toBe('Tue, 31 Dec 2030 23:59:59 +0000');
            });

            it('throws exception on invalid input', function (string $input) {
                expect(fn() => DateTimeRFC2822::fromString($input))
                    ->toThrow(RFC2822DateTimeTypeException::class);
            })->with([
        ...
        describe('tryFromMixed', function () {
            it('returns instance for valid mixed inputs', function (mixed $input, string $expected) {
                ...
                expect($result)->toBeInstanceOf(DateTimeRFC2822::class)
        ...
        describe('tryFromString', function () {
            it('returns instance or default value', function (string $input, bool $isSuccess) {
                $result = DateTimeRFC2822::tryFromString($input);
                if ($isSuccess) {
                    expect($result)->toBeInstanceOf(DateTimeRFC2822::class);
                } else {
                    expect($result)->toBeInstanceOf(Undefined::class);
        ...
        describe('getFormat', function () {
            it('returns RFC2822 format', function () {
                expect(DateTimeRFC2822::getFormat())->toBe(\DATE_RFC2822);
            });
        });
    });
    ...
    describe('Instance Methods', function () {
        ...
        describe('isTypeOf', function () {
            it('returns true when class matches', function () {
                $vo = DateTimeRFC2822::fromString('Thu, 02 Jan 2025 03:04:05 +0000');
                expect($vo->isTypeOf(DateTimeRFC2822::class))->toBeTrue();
            });
            ...
            it('returns true for multiple classNames when one matches', function () {
                $vo = DateTimeRFC2822::fromString('Thu, 02 Jan 2025 03:04:05 +0000');
                expect($vo->isTypeOf('NonExistentClass', DateTimeRFC2822::class, 'AnotherClass'))->toBeTrue();
            });
