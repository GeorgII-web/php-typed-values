...
/**
 * IntegerMinute (Minute number between 0 and 59).
 *
 * Represents an integer constrained to the inclusive range 0..59.
 * Factories accept strictly validated strings and native ints.
 *
 * Example
 *  - $v = IntegerMinute::fromString('45');
 *    $v->value(); // 45
 *  - $v = IntegerMinute::fromInt(9);
 *    (string) $v; // "9"
 *
 * @psalm-immutable
 */
readonly class IntegerMinute extends IntegerTypeAbstract
{
    /** @var int<0, 59> */
    protected int $value;

    /**
     * @throws MinuteIntegerTypeException
     */
    public function __construct(int $value)
    {
        if ($value < 0 || $value > 59) {
            throw new MinuteIntegerTypeException(sprintf('Expected value between 0-59, got "%d"', $value));
        }
    ...
    {
        throw new MinuteIntegerTypeException('Integer type cannot be created from null');
    }
    ...
    {
        throw new MinuteIntegerTypeException('Integer type cannot be converted to null');
    }
