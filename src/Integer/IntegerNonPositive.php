...
/**
 * IntegerNonPositive (Non-positive integer <= 0).
 *
 * Guarantees the wrapped integer is zero or negative. Provides factories from
 * strictly validated string and native int, along with standard formatting.
 *
 * Example
 *  - $v = IntegerNonPositive::fromString('0');
 *    $v->value(); // 0 (int)
 *  - $v = IntegerNonPositive::fromInt(-10);
 *    (string) $v; // "-10"
 *
 * @psalm-immutable
 */
readonly class IntegerNonPositive extends IntegerTypeAbstract
{
    protected int $value;
    ...
    {
        if ($value > 0) {
            throw new NonPositiveIntegerTypeException(sprintf('Expected non-positive integer, got "%d"', $value));
        }
