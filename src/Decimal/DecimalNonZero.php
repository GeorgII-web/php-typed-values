...
/**
 * DECIMAL non-zero value encoded as a string.
 *
 * Accepts canonical decimal strings like "123.0", "-5.0", or "3.14". Zero values
 * like "0.0" are not allowed. The original string is preserved as provided.
 *
 * @psalm-immutable
 */
readonly class DecimalNonZero extends DecimalTypeAbstract
{
    /**
    ...
    public function __construct(string $value)
    {
        ...
        if (preg_match('/^-?0+(\.0+)?$/', $decimal) === 1) {
            throw new NonZeroDecimalTypeException(sprintf('Decimal "%s" is not a non-zero value', $value));
        }
