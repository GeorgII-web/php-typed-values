...
/**
 * Negative float-typed value (< 0.0).
 *
 * Ensures the wrapped float is strictly less than zero. Provides factories
 * from validated string and native float and convenient formatting helpers.
 *
 * Example
 *  - $v = FloatNegative::fromString('0.1');
 *    $v->value(); // 0.1
 *  - $v = FloatNegative::fromFloat(2.5);
 *    (string) $v; // "2.5"
 *
 * @psalm-immutable
 */
readonly class FloatNegative extends FloatTypeAbstract
{
    ...
    {
        if ($value >= 0.0) {
            throw new NegativeFloatTypeException(sprintf('Expected negative float, got "%s"', $value));
        }

        if (is_infinite($value)) {
            throw new NegativeFloatTypeException('Infinite float value');
        }

        if (is_nan($value)) {
            throw new NegativeFloatTypeException('Not a number float value');
        }
