...
/**
 * IntegerTinyUnsigned (Database tiny integer TINYINT unsigned: 0..255).
 *
 * Represents an unsigned tinyint value in the range 0..255, matching common
 * MariaDB/MySQL semantics. Factories accept strictly validated strings and
 * native ints and enforce the bounds.
 *
 * Example
 *  - $v = IntegerTinyUnsigned::fromString('255');
 *    $v->value(); // 255
 *  - $v = IntegerTinyUnsigned::fromInt(0);
 *    (string) $v; // "0"
 *
 * @psalm-immutable
 */
readonly class IntegerTinyUnsigned extends IntegerTypeAbstract
{
    /** @var int<0, 255> */
    protected int $value;

    /**
     * @throws UnsignedTinyIntegerTypeException
     */
    public function __construct(int $value)
    {
        if ($value < 0 || $value > 255) {
            throw new UnsignedTinyIntegerTypeException(sprintf('Expected unsigned tiny integer in range 0..255, got "%d"', $value));
        }
    ...
    {
        throw new UnsignedTinyIntegerTypeException('Integer type cannot be created from null');
    }
    ...
    {
        throw new UnsignedTinyIntegerTypeException('Integer type cannot be converted to null');
    }
