...
describe('DecimalNegative', function () {
    it('accepts valid negative decimal strings and preserves value/toString', function (): void {
        ...
        $b = DecimalNegative::fromString('-123.0');
        $c = DecimalNegative::fromString('-3.14');
        $d = DecimalNegative::fromString('-0.0000001');

        expect($a->value())->toBe('-0.1')
            ->and($a->toString())->toBe('-0.1')
            ->and($b->value())->toBe('-123.0')
            ->and($c->toString())->toBe('-3.14')
            ->and($d->value())->toBe('-0.0000001');
    });

    it('throws on non-negative or malformed decimal strings', function (): void {
        expect(fn() => new DecimalNegative(''))
            ->toThrow(DecimalTypeException::class, 'String "" has no valid decimal value')
            ->and(fn() => DecimalNegative::fromString(' '))
            ->toThrow(DecimalTypeException::class, 'String " " has no valid decimal value')
            ->and(fn() => DecimalNegative::fromString('abc'))
            ->toThrow(DecimalTypeException::class, 'String "abc" has no valid strict decimal value')
            ->and(fn() => DecimalNegative::fromString('.5'))
    ...
    it('tryFromString returns instance for valid and Undefined for invalid', function (): void {
        $ok = DecimalNegative::tryFromString('-42.5');
        $bad = DecimalNegative::tryFromString('nope');
        $zero = DecimalNegative::tryFromString('0.0');

        expect($ok)
            ->toBeInstanceOf(DecimalNegative::class)
            ->and($ok->value())
            ->toBe('-42.5')
            ->and($bad)
    ...
    it('toFloat returns exact float only when string equals (string)(float) cast', function (): void {
        expect(DecimalNegative::fromString('-1.0')->toFloat())->toBe(-1.0)
            ->and(DecimalNegative::fromString('-1.5')->toFloat())->toBe(-1.5);

        expect(fn() => DecimalNegative::fromString('-1.50')->toFloat())
            ->toThrow(DecimalTypeException::class, 'String "-1.50" has no valid strict decimal value')
            ->and(fn() => DecimalNegative::fromString('-2.000')->toFloat())
    ...
    it('tryFromMixed handles valid decimal-like values and invalid mixed inputs', function (): void {
        ...
        $fromString = DecimalNegative::tryFromMixed('-42');
        $fromStringFloat = DecimalNegative::tryFromMixed('-3.1415');
        ...
        $fromStringable = DecimalNegative::tryFromMixed($stringable);

        // invalid inputs
        $fromArray = DecimalNegative::tryFromMixed(['x']);
        $fromNull = DecimalNegative::tryFromMixed(null);
        $fromInt = DecimalNegative::tryFromMixed(123);
        $fromZero = DecimalNegative::tryFromMixed('0.0');
        $fromObject = DecimalNegative::tryFromMixed(new stdClass());

        expect($fromString)->toBeInstanceOf(DecimalNegative::class)
            ->and($fromString->value())->toBe('-42')
            ->and($fromStringFloat)->toBeInstanceOf(DecimalNegative::class)
            ->and($fromStringFloat->value())->toBe('-3.1415')
            ->and($fromStringable)->toBeInstanceOf(DecimalNegative::class)
            ->and($fromStringable->value())->toBe('-5.5')
            ->and($fromArray)->toBeInstanceOf(Undefined::class)
            ->and($fromNull)->toBeInstanceOf(Undefined::class)
            ->and($fromInt)->toBeInstanceOf(Undefined::class)
            ->and($fromZero)->toBeInstanceOf(Undefined::class)
    ...
    it('isUndefined returns false for instances and true for Undefined results', function (): void {
        ...
        $ok = DecimalNegative::fromString('-10.5');

        // Invalid inputs via tryFrom* should return Undefined which reports true
        $u1 = DecimalNegative::tryFromString('abc');
        $u2 = DecimalNegative::tryFromMixed(['x']);
        $u3 = DecimalNegative::tryFromInt(1);
    ...
    it('isTypeOf returns true when class matches', function (): void {
        $v = DecimalNegative::fromString('-10.5');
        expect($v->isTypeOf(DecimalNegative::class))->toBeTrue();
    });
    ...
    it('isTypeOf returns true for multiple classNames when one matches', function (): void {
        $v = DecimalNegative::fromString('-10.5');
        expect($v->isTypeOf('NonExistentClass', DecimalNegative::class, 'AnotherClass'))->toBeTrue();
    });

    it('isEmpty is always false for DecimalNegative', function (): void {
        $d = new DecimalNegative('-1.0');
        expect($d->isEmpty())->toBeFalse()
            ->and($d->isEmpty())->not()->toBeTrue();
    });

    it('covers conversions for DecimalNegative', function (): void {
        expect(DecimalNegative::fromInt(-123)->value())->toBe('-123.0')
            ->and(DecimalNegative::fromFloat(-1.2)->value())->toBe('-1.19999999999999996')
            ->and(DecimalNegative::fromDecimal('-1.23')->value())->toBe('-1.23');

        expect(fn() => DecimalNegative::fromString('true'))->toThrow(DecimalTypeException::class);

        $vInt = DecimalNegative::fromString('-123');
        expect($vInt->toInt())->toBe(-123)
            ->and($vInt->toDecimal())->toBe('-123')
            ->and(fn() => $vInt->toBool())->toThrow(StringTypeException::class);

        expect(fn() => DecimalNegative::fromString('-1.2')->toFloat())->toThrow(StringTypeException::class);
    });

    it('tryFromBool, tryFromFloat, tryFromInt return DecimalNegative for valid inputs', function (): void {
        expect(DecimalNegative::tryFromFloat(-1.2))->toBeInstanceOf(DecimalNegative::class)
            ->and(DecimalNegative::tryFromInt(-123))->toBeInstanceOf(DecimalNegative::class)
            ->and(DecimalNegative::tryFromBool(true))->toBeInstanceOf(Undefined::class)
            ->and(DecimalNegative::tryFromBool(false))->toBeInstanceOf(Undefined::class);
    });

    it('cast float > decimal', function (): void {
        expect(DecimalNegative::fromFloat(-1.5)->toString())->toBe('-1.5')
    ...
    it('cast int > decimal', function (): void {
        expect(DecimalNegative::fromInt(-123)->toString())->toBe('-123.0')
            ->and(DecimalNegative::fromInt(-99999999999999999)->toString())->toBe('-99999999999999999.0')
    ...
    it('can be used with bcmath functions', function (): void {
        $a = DecimalNegative::fromString('-1.23');
        $b = DecimalNegative::fromString('-2.34');
    ...
    it('get round-trips decimal strings without changes', function (string $label, string $value): void {
        $decimal = DecimalNegative::fromString($value);
    ...
    it('rejects invalid decimal strings', function (string $label, string $value): void {
        try {
            DecimalNegative::fromString($value);
    ...
    it('validates very large numbers compatible with bcmath', function (): void {
        ...
        $decimal = DecimalNegative::fromString($large);
 ...
 * @coversNothing
 */
readonly class DecimalNegativeTest extends DecimalNegative
{
...
describe('Throwing static', function () {
    it('DecimalNegative::tryFrom* returns Undefined when exception occurs (coverage)', function (): void {
        expect(DecimalNegativeTest::tryFromBool(true))->toBeInstanceOf(Undefined::class)
            ->and(DecimalNegativeTest::tryFromDecimal('-1.0'))->toBeInstanceOf(Undefined::class)
            ->and(DecimalNegativeTest::tryFromFloat(-1.1))->toBeInstanceOf(Undefined::class)
            ->and(DecimalNegativeTest::tryFromInt(-1))->toBeInstanceOf(Undefined::class)
            ->and(DecimalNegativeTest::tryFromMixed('-1.23'))->toBeInstanceOf(Undefined::class)
            ->and(DecimalNegativeTest::tryFromString('-1.23'))->toBeInstanceOf(Undefined::class);
    });
});
