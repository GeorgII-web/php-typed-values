...
/**
 * IntegerNegative (Negative integer < 0).
 *
 * Guarantees the wrapped integer is negative. Provides factories from
 * strictly validated string and native int, along with standard formatting.
 *
 * Example
 *  - $v = IntegerNegative::fromString('-1');
 *    $v->value(); // -1 (int)
 *  - $v = IntegerNegative::fromInt(-10);
 *    (string) $v; // "-10"
 *
 * @psalm-immutable
 */
class IntegerNegative extends IntegerTypeAbstract
{
    /** @var negative-int
     * @readonly */
    protected int $value;
    ...
    /**
     * @throws NegativeIntegerTypeException
     */
    public function __construct(int $value)
    {
        if ($value >= 0) {
            throw new NegativeIntegerTypeException(sprintf('Expected negative integer, got "%d"', $value));
        }
    ...
    {
        throw new NegativeIntegerTypeException('Integer type cannot be created from null');
    }
    ...
    {
        throw new NegativeIntegerTypeException('Integer type cannot be converted to null');
    }
        ...
        try {
            switch (true) {
                ...
                case is_string($value) || is_object($value) && method_exists($value, '__toString'):
                    return static::tryFromDecimal((string) $value, static::tryFromString((string) $value, $default));
                default:
